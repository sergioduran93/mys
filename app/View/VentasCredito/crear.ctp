...
<div class="row" style="width:90%; margin-left:5%;">
	...
	<?php echo $this->Form->create('VentasCredito',array('class'=>'form-inline ','onsubmit' => 'return itsclicked;'));?>
		<div><h3 class="h3-black"><center>VENTAS DE CREDITO</center></h3></div>
		...
		<div class="bs-callout bs-callout-warning" style="margin: 10px 0px 5px 30px;padding:2px;">
			<h4 style="float:left;">Cliente</h4>
			<?php echo $this->Form->input('otro_remi',array('label'=>'Diferente remitente','type'=>'checkbox','style'=>'margin-left:20px;')); ?>
		    <div class="form-group col-md-12">
		...
		<div class="col-md-12" style="padding:0px;">
			<div class="col-md-6">
				...
				<div class="col-md-12">
					<div class="col-md-12"><?php echo $this->Form->input('barras',array('label'=>'Codigo de Barras:','type'=>'hidden')); ?></div>
				</div>
				<div class="col-md-12">
...
<script>
	$("#btn-submit").click(function(){
		...
		if(ajaxCall != undefined){
			...
			} else if($("#VentasCreditoCrearForm").data('bootstrapValidator').isValid()) {
				itsclicked = true;
				$("#VentasCreditoCrearForm").data('bootstrapValidator').defaultSubmit(); 
				$('#VentasCreditoCrearForm').bootstrapValidator('validate');
				$("#VentasCreditoCrearForm").submit(function(event){
						return;
	...
	viewModel.addUser = function() {
		viewModel.users.push(new User({barras:"", empaques:"", descripcion:"", cantidad:"", valKilo:"", pesoUni:"", largo:"", ancho:"", alto:"", peso:"", pesoVol:"", valor:"", kiloAd:"" }));
		...
		$.each( clientes, function( key, value ) {
			if(clienteSelec == value.Cliente.id){
				if(value.Cliente.activo == "No"){
					...
					$("#VentasCreditoDireccionClien").trigger('change');
					$("#VentasCreditoTelefonoClien").trigger('change');
		...
		select: function( event, ui ) {
			$.each( destinatarios, function( key, value ) {
				if(ui.item.value == value.Destinatario.documento){
					...
					$("#VentasCreditoNombreDest").trigger('change');
					$("#VentasCreditoDireccionDest").trigger('change');
					$("#VentasCreditoTelefonoDest").trigger('change');
						...
						beforeClose: function(){
							...
							$.ajax({
								...
								success: function(response) {
									...
									var ind                = parseInt($("#VentasCreditoDestino").val());
...
$(document).ready(function(){
	$("#selbarra").keypress(function(e){
		if(e.which == 13) {
			...
			$("#VentasCreditoDocumentoClien").val(cl);
			$("#VentasCreditoDocumentoClien").trigger('chosen:updated');
			$("#VentasCreditoDocumentoClien").trigger('change');
			//$("#VentasCreditoDestino").trigger('chosen:activate');
			$("#VentasCreditoDocumento1").focus();
		}
	});
