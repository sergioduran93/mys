...
<div class="row" style="width:90%; margin-left:5%;">
	...
	<?php echo $this->Form->create('Recibo',array('class'=>'form-inline','onsubmit' => 'return itsclicked;'));?>
	    ...
	    <div class="form-group col-md-12">
			...
			<div class="col-md-3" style="margin-top:25px;"><?php echo $this->Form->input('provisional',array('label'=>'Recibo provisional: ','type'=>'checkbox','style'=>'height:25px;width:25px')); ?></div>
		</div>
	    <div class="form-group col-md-12">
	    	...
	    	<div class="col-md-4"><?php echo $this->Form->input('negociador',array('label'=>'Negociador CC: ','class'=>'provisional','type'=>'text','placeholder'=>'Seleccione')); ?></div>
       		<div class="col-md-4"><?php echo $this->Form->input('negociador_nom',array('label'=>'Negociador Nombre: ','class'=>'provisional','type'=>'text','placeholder'=>'Seleccione')); ?></div>
		</div>
	    <div class="form-group col-md-12">
...
<script>
	...
	var negociadores    = <?php echo json_encode($negociadores); ?>;
	...
	$.each( negociadores, function( key, value ) {
		negoDoc[key] = value.Auxiliar.documento;
		negoNom[key] = value.Auxiliar.nombre;
	});
...
$(function() {
	...
	$("#ReciboNegociador").autocomplete({
		...
		select: function( event, ui ) {
			$.each( negociadores, function( key, value ) {
				if(ui.item.value == value.Auxiliar.documento){
					$("#ReciboNegociadorNom").val(value.Auxiliar.nombre);
				}
			});
		}
	});
	$("#ReciboNegociadorNom").autocomplete({
		...
		select: function( event, ui ) {
			$.each( negociadores, function( key, value ) {
				if(ui.item.value == value.Auxiliar.nombre){
					$("#ReciboNegociador").val(value.Auxiliar.documento);
				}
			});
		}
	});
