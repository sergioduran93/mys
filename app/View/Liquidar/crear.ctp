...
<div class="row" style="width:90%; margin-left:5%;">
	...
	<?php echo $this->Form->create('Liquidar',array('class'=>'form-inline','onsubmit' => 'return itsclicked;'));?>
		<div><h3><center>COTIZAR VENTAS</center></h3></div>
	    ...
	    <div class="form-group col-md-12">
	    	<div class="col-md-4"><?php echo $this->Form->input('documento',array('label'=>'Identificación','type'=>'text','placeholder'=>'Seleccione el cliente')); ?></div>
	    	<div class="col-md-4"><?php echo $this->Form->input('nombre',array('label'=>'Nombre','type'=>'text')); ?></div>
	    	<div class="col-md-4"><?php echo $this->Form->input('telefono',array('label'=>'Teléfono','type'=>'text','placeholder'=>'Seleccione el cliente')); ?></div>
		</div>
		<div class="form-group col-md-12">
	    	<div class="col-md-4"><?php echo $this->Form->input('email',array('label'=>'Email:','type'=>'text')); ?></div>
	    	<div class="col-md-4"><?php echo $this->Form->input('telefono2',array('label'=>'Teléfono2:','type'=>'text')); ?></div>
	    	<div class="col-md-4"><?php echo $this->Form->input('fax',array('label'=>'Fax:','type'=>'text')); ?></div>
		</div>
	    <div class="form-group col-md-12">
		...
		<div class="col-md-12" style="padding:0px;">
			<div class="form-group col-md-6">
				<div class="col-md-12">
					<div class="col-md-4"><?php echo $this->Form->input('fecha',array('label'=>'Duración:','type'=>'text','placeholder'=>'AAAA-MM-DD')); ?></div>
					<div class="col-md-4 descu"><?php echo $this->Form->input('descPorcen',array('label'=>'Descuento (%):','type'=>'text','placeholder'=>'0','default'=>'0')); ?></div>
					<div class="col-md-4 descu"><?php echo $this->Form->input('descValor',array('label'=>'Negociar por:','type'=>'text','placeholder'=>'0')); ?></div>
				</div>
...
<script>
	...
	function calcularTotal(){
		...
		opT         = op;
		var descPorcen = $("#LiquidarDescPorcen").val();
		if(descPorcen > descuentoMax){
			descPorcen = descuentoMax;
		}
		op     = op-(op*(descPorcen/100));

		op     = round100(op);
		valorT = op;
		$("#LiquidarDescValor").val(op.formatMoney(0,',','.'));
	...
	<?php if($usuario_actual['cliente_id'] == ""){
		echo "var clienteUserId = '1';";
	}else{
		echo "var clienteUserId =".$usuario_actual['cliente_id'].";";
	} ?>
	...
	var pesoKiloAd             = 0;
$(function() {
	$("#LiquidarDescValor").keyup(function(){
		var strNum = $(this).val();
...
$(document).ready(function(){
	if(roleId == 4){
		$.each( clientes, function( key, value ) {
			if(clienteUserId == value.Cliente.id){
				if(value.Cliente.activo == "No"){
					$("#LiquidarDocumento").val("");
					$("#LiquidarCliente").val("");
					$("#LiquidarNombre").val("");
					...
					$("#LiquidarCliente").attr('readonly','readonly');
					$("#LiquidarDocumento").attr('readonly','readonly');
					$("#LiquidarNombre").attr('readonly','readonly');
					$("#LiquidarDireccion").attr('readonly','readonly');
					$("#LiquidarTelefono").attr('readonly','readonly');
					$("#LiquidarTelefono2").attr('readonly','readonly');
					$("#LiquidarEmail").attr('readonly','readonly');
					$("#LiquidarFax").attr('readonly','readonly');
				}
			}
		});
	}
	...
	$("#LiquidarDescPorcen").change(function(){
		var descPorcen = $(this).val();
		if(descPorcen > descuentoMax){
			descPorcen = descuentoMax;
			$("#LiquidarDescPorcen").val(descuentoMax);
		}
		calcularTotal();
	});
	$("#LiquidarDescValor").blur(function(){
		var strNum       = $(this).val();
		var descValor    = parseFloat(strNum.replace(/,/g,""));
		var valorMaxDesc = opT-(opT*(descuentoMax/100));
		if(descValor < valorMaxDesc){
			descValor = valorMaxDesc;
			$("#LiquidarDescValor").val(valorMaxDesc.formatMoney(0,',','.'));
			$("#LiquidarDescPorcen").val(descuentoMax);
		} else {
			$("#LiquidarDescPorcen").val(100-((descValor*100)/opT));
		}
		calcularTotal();
	});
			...
			if($(this).val() != ""){
				...
				$.ajax({
					...
					success: function(response) {
						...
						ajaxCall = response;
						descuentoMax = ajaxCall.Descuento;
						$.fancybox.hideLoading();
						$.fancybox.helpers.overlay.close();
					},
					error: function(e) {
