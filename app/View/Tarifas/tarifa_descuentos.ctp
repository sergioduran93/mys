...
<div class="row" style="width:90%; margin-left:5%;">
	...
	<?php echo $this->Form->create('Convenio',array('class'=>'form-inline'));?>
		<div><h3><center>DESCUENTOS (GENERAL)</center></h3></div>
	    <fieldset>
...
$(document).ready(function(){
	...
	$("#ConvenioRegion").change(function () {
		...
		$("#ConvenioTarifaDescuentosForm").data('bootstrapValidator').resetForm();
		$('#ConvenioTarifaDescuentosForm').bootstrapValidator('validate');
	});
	...
	$('#tabla_id').on('click', 'tr', function(event) {
		...
		if (null != aData) {
			...
			$.ajax({
				type: 'json',
				url: fullpath+"tarifas/getDescuentos/1/"+origen+"/"+destino,
