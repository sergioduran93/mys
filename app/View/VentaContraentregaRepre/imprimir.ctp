...
for ($i=0; $i < $n; $i++) { 
	...
	if($otro_remi){
		...
		$pdf->MultiCell(50, 1, $remitente, 0, 'C', 0, 0, '22', '51', true);
	...
	if($hoja[$i] == '**DESTINATARIO**') {
		$pdf->barraY = 81;
		$pdf->Line(5, 5, 5, ($altoM-40+$remi), null); //IZQ
		$pdf->Line(($largoM-5), 5, ($largoM-5), ($altoM-40+$remi), null);  //DER
		$pdf->Line(5, ($altoM-40+$remi), ($largoM-5), ($altoM-40+$remi), null); //ABA
	} else if($hoja[$i] == '**PRUEBA DE ENTREGA**') {
		$pdf->SetFont('Times','B',14);
		$pdf->Line(5, 5, 5, ($altoM-25+$remi), null); //IZQ
		$pdf->Line(($largoM-5), 5, ($largoM-5), ($altoM-25+$remi), null);  //DER
		$pdf->Line(5, ($altoM-25+$remi), ($largoM-5), ($altoM-25+$remi), null); //ABA
		$pdf->barraY = 82;
		$pdf->barraX = 19;
		$pdf->Line(110, ($altoM-25+$remi), 110, 81+$remi, null);
		$pdf->Line(5, 96+$remi, 110, 97+$remi, null);

		$pdf->Line(131, 86+$remi, $largoM-8, 86+$remi, null);
		$pdf->Line(137, 91+$remi, $largoM-8, 91+$remi, null);
		$pdf->Line(124, 96+$remi, $largoM-8, 96+$remi, null);
		$pdf->Line(132, 101+$remi, 164, 101+$remi, null);
		$pdf->Line(182, 101+$remi, $largoM-8, 101+$remi, null);
		$pdf->Line(142, 106+$remi, $largoM-8, 106+$remi, null);
		$pdf->Line(131, 111+$remi, $largoM-8, 111+$remi, null);
	//	$pdf->Line(170, 97, 205, 97, null);
	//	$pdf->Line(100, 102, 150, 102, null);
		$pdf->SetFont('Times','BI',13);

		$pdf->MultiCell(60, 1, "FIRMA:", 0, 'L', 0, 0, '111', 81+$remi, true);
		$pdf->MultiCell(60, 1, "NOMBRE:", 0, 'L', 0, 0, '111', 86+$remi, true);
		$pdf->MultiCell(60, 1, "C.C.:", 0, 'L', 0, 0, '111', 91+$remi, true);
		$pdf->MultiCell(60, 1, "FECHA:", 0, 'L', 0, 0, '111', 96+$remi, true);
		$pdf->MultiCell(60, 1, "HORA:", 0, 'L', 0, 0, '165', 96+$remi, true);
		$pdf->MultiCell(60, 1, "TELÉFONO:", 0, 'L', 0, 0, '111', 101+$remi, true);
		$pdf->MultiCell(60, 1, "CARGO:", 0, 'L', 0, 0, '111', 106+$remi, true);

		$pdf->MultiCell(100, 1, "***DEVOLVER FIRMADO***", 0, 'L', 0, 0, '130', 112+$remi, true);

	} else {
		$pdf->Line(5, 5, 5, ($altoM-25+$remi), null); //IZQ
		$pdf->Line(($largoM-5), 5, ($largoM-5), ($altoM-25+$remi), null);  //DER
		$pdf->Line(5, ($altoM-25+$remi), ($largoM-5), ($altoM-25+$remi), null); //ABA
		$pdf->Line(5, 96+$remi, ($largoM-5), 97+$remi, null);
	...
	if($hoja[$i] == '**DESTINATARIO**') {
		...
		$pdf->MultiCell(80, 1,  $tipoHoja, 0, 'L', 0, 0, '6', 97+$remi, true);
		$pdf->MultiCell(120, 1, $resolucion, 0, 'L', 0, 0, '6', 102+$remi, true);
