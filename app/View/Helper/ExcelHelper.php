...
App::import('Vendor', 'ExcelWriter',array('file'=>'excel_writer'.DS.'ExcelWriter.php'));
App::import('Vendor', 'OleRoot',array('file'=>'ole'.DS.'OleRoot.php'));
App::import('Vendor', 'OleFile',array('file'=>'ole'.DS.'OleFile.php'));
...
class ExcelHelper extends ExcelWriter {
   ...
   */
  public function ExcelHelper($filename = 'data.xls') {
   ...
   */
  private function _getFormatArray($params = NULL) {
   ...
   */
  private function _GetExcelEpoch() {
    return $this->GregorianToJD(1,1,1900); // Windows Excel epoch
  }
   ...
   */
  public function &AddWorksheet($name = NULL) {
   ...
   */
  public function AddFormat($params) {
   ...
   */
  public function MysqlDatetimeToExcel($datetime) {
    $tmp = explode(" ", $datetime);
    $date = explode("-", $tmp[0]);
    if(isset($tmp[1])) $time = explode(":", $tmp[1]);
    $date1 = $this->GregorianToJD($date[1],$date[2],$date[0]);
  ...
  public function GregorianToJD ($month,$day,$year) {
     if ($month > 2) {
         $month = $month - 3;
     } else {
         $month = $month + 9;
   ...
   * @param float            Excel datetime value.
   */
  public function TimestampToExcel($timestamp) {
   ...
   */
  public function write(&$worksheet, $row, $col, $token, $format = 0) {
    return $worksheet->write($row, $col, $token, $this->formats[$format]);
  }

  public function setMerge(&$worksheet,$first_row, $first_col, $last_row, $last_col){
    return $worksheet->setMerge($first_row, $first_col, $last_row, $last_col);
  }

  public function mergeCells(&$worksheet,$first_row, $first_col, $last_row, $last_col){
    return $worksheet->setMerge($first_row, $first_col, $last_row, $last_col);
  }


  public function hideGridLines(&$worksheet){
    return $worksheet->hideGridLines();
  }
  /**
  ...
  public function OutputFile() {
    $this->workbook->Close();
    $this->workbook->filename = 'Reporte_SIPEX';
