...
<div class="row" style="width:90%; margin-left:5%;">
	...
	<?php echo $this->Form->create('Venta',array('class'=>'form-inline','onsubmit' => 'return itsclicked;'));?>
		<div><h3><center>VENTAS DE CONTADO</center></h3></div>
		...
		<div class="form-group col-md-12">
			<div class="col-md-2" style="width:14.28%;"><?php echo $this->Form->input('facturacion',array('label'=>'Factura No.: ','type'=>'text','readonly'=>'readonly','default'=>$facturacion)); ?></div>
			<div class="col-md-2" style="width:14.28%;"><?php echo $this->Form->input('remesa',array('label'=>'Remesa No.: ','type'=>'text','readonly'=>'readonly','default'=>$remesa)); ?></div>
			<div class="col-md-2" style="width:14.28%;"><?php echo $this->Form->input('selbarra',array('label'=>'Cod. Barra','type'=>'text','id'=>"selbarra")); ?></div>
			<div class="col-md-2" style="width:14.28%;"><?php echo $this->Form->input('documento1',array('label'=>'Documento Ref1.: ','type'=>'text')); ?></div>
			<div class="col-md-2" style="width:14.28%;"><?php echo $this->Form->input('tipo',array('label'=>'Tipo de Doc: : ','type'=>'text')); ?></div>
			<div class="col-md-2" style="width:14.28%;"><?php echo $this->Form->input('documento2',array('label'=>'Documento Ref2.: ','type'=>'text')); ?></div>
			<div class="col-md-2" style="width:14.28%;"><?php echo $this->Form->input('documento3',array('label'=>'Documento Ref3.: ','type'=>'text')); ?></div>
		</div>
...
$(function() {
	...
	$("#VentaDocumentoClien").autocomplete({
		source: clientesIdentificacion,
		select: function( event, ui ) {
			$.each( clientes, function( key, value ) {
				if(ui.item.value == value.Cliente.documento){
					if(value.Cliente.activo == "No"){
						$("#VentaDocumentoClien").val("");
						$("#VentaCliente").val("");
						$("#VentaNombreClien").val("");
						$("#VentaDireccionClien").val("");
						$("#VentaTelefonoClien").val("");
						$("#VentaTelefono2Clien").val("");
						$("#VentaEmailClien").val("");
						$("#VentaFaxClien").val("");
						...
						$("#VentaCliente").val(value.Cliente.id);
						$("#VentaNombreClien").val(value.Cliente.listNombre);
						$("#VentaDireccionClien").val(value.Cliente.direccion);
						$("#VentaTelefonoClien").val(value.Cliente.telefono);
						$("#VentaTelefono2Clien").val(value.Cliente.telefono2);
						$("#VentaEmailClien").val(value.Cliente.email);
						$("#VentaFaxClien").val(value.Cliente.fax);
						...
						$("#VentaDocumentoRemi").autocomplete('option', 'source', remitentIdentificacion);
						$("#VentaNombreRemi").autocomplete('option', 'source', remitentNombre);
					}
					$("#VentaDestino").val("");
					$("#VentaDestino").trigger("chosen:updated");
				}
			});
			$("#VentaNombreClien").trigger('val');
			$("#VentaDocumentoClien").trigger('val');
			$("#VentaDireccionClien").trigger('val');
			$("#VentaTelefonoClien").trigger('val');
		},
		change: function( event, ui ){
			...
			if(flag){
				if(confirm("El cliente NO existe, ¿Desea registrarlo?")){
					var docAct  = $("#VentaDocumentoClien").val();
					var nomAct  = $("#VentaNombreClien").val();
					var nomAct2 = "";
					if(docAct == ""){
						docAct = "*";
					}
					...
					$.fancybox.open({
						...
						beforeClose: function(){
							...
							$("#VentaDocumentoClien").val(arrayInfo[0]);
							$("#VentaNombreClien").val(arrayInfo[1]);
							$("#VentaDireccionClien").val(arrayInfo[2]);
							$("#VentaTelefonoClien").val(arrayInfo[3]);
							$("#VentaTelefono2Clien").val(arrayInfo[4]);
							$("#VentaEmailClien").val(arrayInfo[5]);
							$("#VentaFaxClien").val(arrayInfo[6]);
							remitentIdentificacion = new Array();
							remitentNombre         = new Array();
							if(arrayInfo[7] != null){
								var i = 0;
			...
			$.each( clientes, function( key, value ) {
				if(ui.item.value == value.Cliente.listNombre){
					if(value.Cliente.activo == "No"){
						...
						clienteId = value.Cliente.id;
						$("#VentaCliente").val(value.Cliente.id);
						$("#VentaDocumentoClien").val(value.Cliente.documento);
						$("#VentaDireccionClien").val(value.Cliente.direccion);
						$("#VentaTelefonoClien").val(value.Cliente.telefono);
						$("#VentaTelefono2Clien").val(value.Cliente.telefono2);
						$("#VentaEmailClien").val(value.Cliente.email);
	...
	$("#VentaDocumentoRemi").autocomplete({
		...
		select: function( event, ui ) {
			$.each( remitentes, function( key, value ) {
				if(ui.item.value == value.Remitente.documento){
					...
					$("#VentaRemitente").val(value.Remitente.id);
					$("#VentaNombreRemi").val(value.Remitente.nombre);
					$("#VentaDireccionRemi").val(value.Remitente.direccion);
					$("#VentaTelefonoRemi").val(value.Remitente.telefono);
					$("#VentaCelularRemi").val(value.Remitente.celular);
					$("#VentaEmailRemi").val(value.Remitente.email);
		...
		select: function( event, ui ) {
			$.each( remitentes, function( key, value ) {
				if(ui.item.value == value.Remitente.nombre){
					remitenteId = value.Remitente.id;
					$("#VentaRemitente").val(value.Remitente.id);
					$("#VentaDocumentoRemi").val(value.Remitente.documento);
					$("#VentaDireccionRemi").val(value.Remitente.direccion);
					$("#VentaTelefonoRemi").val(value.Remitente.telefono);
					$("#VentaCelularRemi").val(value.Remitente.celular);
					$("#VentaEmailRemi").val(value.Remitente.email);
					$("#VentaNombreCont").val(value.Remitente.contacto);
					$("#VentaTelefonoCont").val(value.Remitente.telefono_cont);
				}
			});
		}
	});

	$("#VentaDocumentoDest").autocomplete({
		...
		select: function( event, ui ) {
			$.each( destinatarios, function( key, value ) {
				if(ui.item.value == value.Destinatario.documento){
					...
					$("#VentaDestinatario").val(value.Destinatario.id);
					$("#VentaNombreDest").val(value.Destinatario.listNombre);
					$("#VentaDireccionDest").val(value.Destinatario.direccion);
					$("#VentaTelefonoDest").val(value.Destinatario.telefono);
					$("#VentaTelefono2Dest").val(value.Destinatario.telefono2);
					$("#VentaEmailDest").val(value.Destinatario.email);
					$("#VentaFaxDest").val(value.Destinatario.fax);
				}
			});
			$("#VentaNombreDest").trigger('val');
			$("#VentaDireccionDest").trigger('val');
			$("#VentaTelefonoDest").trigger('val');
		}
	});
	$("#VentaNombreDest").autocomplete({
		...
		select: function( event, ui ) {
			$.each( destinatarios, function( key, value ) {
				if(ui.item.value == value.Destinatario.listNombre){
					destinatarioId = value.Destinatario.id;
					$("#VentaDestinatario").val(value.Destinatario.id);
					$("#VentaDocumentoDest").val(value.Destinatario.documento);
					$("#VentaDireccionDest").val(value.Destinatario.direccion);
					$("#VentaTelefonoDest").val(value.Destinatario.telefono);
					$("#VentaTelefono2Dest").val(value.Destinatario.telefono2);
					$("#VentaEmailDest").val(value.Destinatario.email);
		...
		select: function( event, ui ) {
			$.each( contactosArray, function( key, value ) {
				if(ui.item.value == value){
					$("#VentaContactoTel").val(contactosArrayTel[key]);
				}
			});
		}
	});
...
$(document).ready(function(){
		...
		if(e.which == 13) {
			...
			$.each( clientes, function( key, value ) {
				if(value.Cliente.id == cl){
					$("#VentaDocumentoClien").val(clientesIdentificacion[key]);
					$("#VentaDocumentoClien").trigger("change");
					$("#VentaNombreClien").trigger("change");
					$("#VentaDireccionClien").trigger("change");
					$("#VentaTelefonoClien").trigger("change");
				}
			});
			$("#VentaDocumento1").focus();
		}
	});
	...
	$('#VentaCrearForm').bootstrapValidator({
		feedbackIcons: {
	...
	var indClien = 0;
	$.each( clientes, function( key, value ) {
		clientesIdentificacion[indClien] = value.Cliente.documento;
		clientesNombre[indClien]         = value.Cliente.listNombre;
		indClien++;
	});
	var indexTipo = 0;
	...
	viewModel.addUser();
	$("#VentaFecha").datepicker({minDate: 0});

	$("#VentaOtroRemi").change(function(){
		if($(this).is(':checked')){
			$("#div-remitente").show();
		} else {
			$("#div-remitente").hide();
	...
	$("#btn-limpiar").click(function(){
		...
		$("#VentaDestino").val("");
		$("#VentaOrigen").val("");
		$("#VentaDocumentoRemi").val("");
		$("#VentaTelefono").val("");
		$("#VentaTelefono").trigger("chosen:updated");
		$("#VentaOrigen").trigger("chosen:updated");
		$("#VentaDestino").trigger("chosen:updated");
		clienteId      = 1;
		remitenteId    = 0;
		destinatarioId = 0;
...
/*
	$("#VentaDeclarado").number( true, 0 );
*/
	$("#VentaDeclarado").keyup(function(){
		var strNum = $(this).val();
		var num    = parseFloat(strNum.replace(/,/g,""));
		num        = num.formatMoney(0,',','.');
		$(this).val(num);
		$("#VentaDeclarado").trigger("val");
	});

	$("#VentaDeclarado").blur(function(){
		seguro = 0;
		...
		if($(this).val() == "Si"){
			...
			$.each( ajaxCall.Tarifa, function( key, value ) {
				if(4 == value.Tarifa.empaque_id){
					...
					$("#VentaDevolverFirmado").val(costoDevol);
				}
			});
			if(!existeDevol){
	...
	$("#VentaOrigen").val(ciudad);
	$("#VentaOrigen").trigger("chosen:updated");



	$("#VentaNombreClien").change(function(){
		var flag = true;
		var docuCli = $(this).val();
		$.each( clientes, function( key, value ) {
	...
	$("#VentaNombreDest").change(function(){
		var flag = true;
		...
		if(flag){
			if(confirm("El destinatario NO existe, ¿Desea registrarlo?")){
				var docAct  = $("#VentaDocumentoDest").val();
				var nomAct  = $("#VentaNombreDest").val();
				var nomAct2 = "";
				if(docAct == ""){
					docAct = "/*";
				} else {
				...
				$.fancybox.open({
					...
					beforeClose: function(){
						...
						arrayInfo = JSON.parse(retorno);
						$("#VentaDocumentoDest").val(arrayInfo[0]);
						$("#VentaNombreDest").val(arrayInfo[1]);
						$("#VentaDireccionDest").val(arrayInfo[2]);
						$("#VentaTelefonoDest").val(arrayInfo[3]);
						$("#VentaTelefono2Dest").val(arrayInfo[4]);
						$("#VentaEmailDest").val(arrayInfo[5]);
						$("#VentaFaxDest").val(arrayInfo[6]);

						$.ajax({
							type: 'json',
							url: fullpath+"ventas/getDestinatarios",
							beforeSend: function(xhr) {
								xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
							},
							success: function(response) {
								response       = JSON.parse(response);
								destinatariosN = response.Nombre;
								destinatarios  = response.Todo;
								var ind                = parseInt($("#VentaDestino").val());
								...
								$("#VentaDocumentoDest").autocomplete('option', 'source', destinatIdentificacion);
								$("#VentaNombreDest").autocomplete('option', 'source', destinatNombre);
							},
							error: function(e) {
	...
	$("#VentaDocumentoDest").change(function(){
		var flag = true;
		var docuDest = $(this).val();
		$.each( destinatarios, function( key, value ) {
	...
	/*
	$('#VentaDocumentoClien').JCombo({
		'parentId': 'VentaDocumentoClien',
		'nodeId'  : 'VentaDocumentoRemi',
		'data'    : selectDep
	});*/
	...
	$("#VentaDestino").change(function(){
		if($("#VentaOrigen").val() == ""){
			...
			if($(this).val() != ""){
				...
				$("#VentaNombreDest").autocomplete('option', 'source', destinatNombre);
				$("#kiloAdSpan").val("");
				$("#VentaDestinatario").val("");
				$("#VentaNombreDest").val("");
				$("#VentaDocumentoDest").val("");
				$("#VentaDireccionDest").val("");
				$("#VentaTelefonoDest").val("");
				$("#VentaTelefono2Dest").val("");
				$("#VentaEmailDest").val("");
				$("#VentaFaxDest").val("");
				origenId  = $("#VentaOrigen").val();
				destinoId = $("#VentaDestino").val();
				$("#VentaDeclarado").val("");
				...
				$("#VentaFirmado").val("No");
				calcularTotal();
				var clienteDocumActual = $("#VentaDocumentoClien").val();
				var flag = true;
				$.each( clientes, function( key, value ) {
