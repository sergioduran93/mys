...
<div class="row" style="width:90%; margin-left:5%;">   
	<br>
	<div><h3><center>DESPACHOS PARA EL REPRESENTANTE</center></h3></div>
    <fieldset>
	...
	<div class="bs-callout bs-callout-info" style="margin:2px 0px 2px 0px;padding: 0px 5px;">
		<h4>Representante</h4>
		<div class="form-group col-md-12">
			<div class="col-md-3"><?php echo $this->Form->input('repreD',array('label'=>'Cedula: ','type'=>'select','options'=>$repreD,'empty'=>"")); ?></div>
			<div class="col-md-3"><?php echo $this->Form->input('repreT',array('label'=>'Nombre: ','type'=>'select','options'=>$repreT,'empty'=>"")); ?></div>
...
<script>
	...
	var repreA  = <?php echo json_encode($repreA); ?>;
...
$(document).ready(function(){
	...
	$("#VentaRepreT").change(function(){
		...
		if(sel == ""){
			...
			$.each(repreA, function( key, value ) {
				if(value.Representante.id == sel){
					$("#VentaTelefono").val(value.Representante.telefono1);
					$("#VentaDireccion").val(value.Representante.direccion);
				}
			});
			oTable.fnFilter($("#VentaRepreT option:selected").text(), 9);
		}
		$("#VentaRepreD").val(sel);
		$("#VentaRepreD").trigger("chosen:updated");
	});
	$("#VentaRepreD").change(function(){
		var sel = $(this).val();
		if(sel == ""){
			oTable.fnFilter("",8);
			$("#VentaTelefono").val("");
			...
			oTable.fnFilter($("#VentaRepreD option:selected").text(), 8);
		}
		$("#VentaRepreT").val(sel);
		$("#VentaRepreT").trigger("chosen:updated");
	});
	...
	oTable = $('#tabla_id').dataTable( {
		...
		"sAjaxSource": webroot+'sources/despacho_repre.txt',
		...
		"aoColumnDefs": [
			...
			{ "sWidth": "100px", "aTargets": [2,4,6,14,15,16,17,18] },
