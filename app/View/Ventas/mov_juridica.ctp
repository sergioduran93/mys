...
<div class="row" style="width:90%; margin-left:5%;">   
	<br>
	<div><h3><center>MOVILIZACIÓN POR TRANSPORTADORA</center></h3></div>
    <fieldset>
	...
	<div class="bs-callout bs-callout-info" style="margin:2px 0px 2px 0px;padding: 0px 5px;">
		...
		<div class="form-group col-md-12">
			<div class="col-md-3"><?php echo $this->Form->input('transpD',array('label'=>'NIT: ','type'=>'select','options'=>$transpD,'empty'=>"")); ?></div>
			<div class="col-md-3"><?php echo $this->Form->input('transpT',array('label'=>'Nombre: ','type'=>'select','options'=>$transpT,'empty'=>"")); ?></div>
	    ...
	    <table id="tabla_id">
			<thead>
				...
				<tr>
					...
					<th>Doc. Ref1</th>
					<th>Transportadora NIT</th>
					<th>Transportadora Nombre</th>
					<th>Origen</th>
					<th>Destino</th>
...
<script>
	...
	var transpA  = <?php echo json_encode($transpA); ?>;
...
$(document).ready(function(){
	...
	$("#descargar").click(function(){
		...
		$.ajax({
			method : "POST",
			url: url+"/downMovJuridica",
			data: $("#VentaMovJuridicaForm").serializeArray(),
	...
	$("#VentaTranspT").change(function(){
		...
		if(sel == ""){
			...
			$.each(transpA, function( key, value ) {
				if(value.Transportadora.id == sel){
					$("#VentaTelefono").val(value.Transportadora.telefono1);
					$("#VentaDireccion").val(value.Transportadora.direccion);
				}
			});
			oTable.fnFilter($("#VentaTranspT option:selected").text(), 8);
		}
		$("#VentaTranspD").val(sel);
		$("#VentaTranspD").trigger("chosen:updated");
	});
	$("#VentaTranspD").change(function(){
		var sel = $(this).val();
		if(sel == ""){
			oTable.fnFilter("",7);
			$("#VentaTelefono").val("");
			...
			oTable.fnFilter($("#VentaTranspD option:selected").text(), 7);
		}
		$("#VentaTranspT").val(sel);
		$("#VentaTranspT").trigger("chosen:updated");
	});
	...
	oTable = $('#tabla_id').dataTable( {
		...
		"sAjaxSource": webroot+'sources/mov_transportadora.txt',
		...
		"aoColumnDefs": [
			...
			{ "sClass": "unidadClass", "aTargets": [13] },
			{ "sClass": "fleteClass", "aTargets": [17] },
			{ "sClass": "seguroClass", "aTargets": [18] }
		],
		"oColReorder": {
            "iFixedColumns": 1
        }
