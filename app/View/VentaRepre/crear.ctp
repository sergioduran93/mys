...
<div class="row" style="width:90%; margin-left:5%;">
	...
	<?php echo $this->Form->create('VentaRepre',array('class'=>'form-inline','onsubmit' => 'return itsclicked;'));?>
		<div><h3><center>VENTAS DE CONTADO (REPRESENTANTE)</center></h3></div>
		...
		<div class="col-md-12" style="padding:0px;">
			...
			<div class="bs-callout bs-callout-info" style="margin: 0px 0px 5px 30px;padding:2px;">
				...
				<div style="float:right;">
					<h4 style="float:left">Imprimir Recibo:</h4>
					<?php echo $this->Form->input('checkImpRemitente',array('label'=>'Remitente','type'=>'checkbox','style'=>'float:left;margin-left:10px;')); ?>
					<?php echo $this->Form->input('checkImpArchivo',array('label'=>'Archivo','type'=>'checkbox','style'=>'float:left;margin-left:10px;')); ?>
				</div>
				<div class="form-group col-md-12">
					<div class="col-md-3"><?php echo $this->Form->input('destino_recibo',array('class'=>'actualizaDiv','label'=>'Destino: ','type'=>'select','options'=>$destinos,'empty'=>'')); ?></div>
					<div class="col-md-3 divNegoc"><?php echo $this->Form->input('negociador',array('class'=>'actualizaDiv','label'=>'Conductor C.C.: ','type'=>'text','placeholder'=>'Seleccione')); ?></div>
					<div class="col-md-3 divNegoc"><?php echo $this->Form->input('negociador_nom',array('class'=>'actualizaDiv','label'=>'Nombre: ','type'=>'text','placeholder'=>'Seleccione')); ?></div>
					<div class="col-md-3 divRazon hidden"><?php echo $this->Form->input('razon',array('class'=>'actualizaDiv','label'=>'Razón social: ','type'=>'text','placeholder'=>'Seleccione')); ?></div>
					<div class="col-md-3"><?php echo $this->Form->input('documento',array('class'=>'actualizaDiv','label'=>array('id'=>'idDocumento','text'=>'Placa: '),'type'=>'text','placeholder'=>'Seleccione')); ?></div>
				</div>
				<div class="form-group col-md-12">
...
$(function() {
	...
	$("#VentaRepreDocumento").autocomplete({
		source: placas
	});
	$("#VentaRepreNegociador").autocomplete({
		source: condDoc,
		select: function( event, ui ) {
			$.each( conductores, function( key, value ) {
				if(ui.item.value == value.Conductor.identificacion){
					$("#VentaRepreNegociadorNom").val(value.Conductor.listNombre);
				}
			});
		}
	});
	$("#VentaRepreNegociadorNom").autocomplete({
		source: condNom,
		select: function( event, ui ) {
			$.each( conductores, function( key, value ) {
				if(ui.item.value == value.Conductor.listNombre){
					$("#VentaRepreNegociador").val(value.Conductor.identificacion);
				}
			});
		}
	});
...
$(document).ready(function(){
		...
		if(e.which == 13) {
			...
			$.each( clientes, function( key, value ) {
				if(value.Cliente.id == cl){
					$("#VentaRepreDocumentoClien").val(clientesIdentificacion[key]);
					$("#VentaRepreDocumentoClien").trigger("change");
					$("#VentaRepreNombreClien").trigger("change");
					$("#VentaRepreDireccionClien").trigger("change");
					$("#VentaRepreTelefonoClien").trigger("change");
				}
			});
			$("#VentaRepreDocumento1").focus();
		}
	});
	...
	$('#VentaRepreCheckNatural').change(function(){
		var flag = $('#VentaRepreCheckNatural').prop('checked');
		$("#VentaRepreCheckJuridica").prop('checked',!flag);
			if(flag){
					...
					select: function( event, ui ) {
						$.each( transportadoras, function( key, value ) {
							if(ui.item.value == value.Transportadora.nit){
								$("#VentaRepreRazon").val(value.Transportadora.razon);
								if(value.Transportadora.credito == 'Si'){
									$("#VentaRepreFormaPago").val('Credito');
								} else {
									$("#VentaRepreFormaPago").val('Contado');
								}
							}
						});
					}
				});
				$("#VentaRepreRazon").autocomplete({
					source: transRazon,
					select: function( event, ui ) {
						$.each( transportadoras, function( key, value ) {
							if(ui.item.value == value.Transportadora.razon){
								$("#VentaRepreDocumento").val(value.Transportadora.nit);
								if(value.Transportadora.credito == 'Si'){
									$("#VentaRepreFormaPago").val('Credito');
								} else {
									$("#VentaRepreFormaPago").val('Contado');
	...
	$('#VentaRepreCheckJuridica').change(function(){
		var flag = $('#VentaRepreCheckJuridica').prop('checked');
		$("#VentaRepreCheckNatural").prop('checked',!flag);
		if(!flag){
			$(".divRazon").addClass('hidden');
			$(".divNegoc").removeClass('hidden');
			$(".actualizaDiv").val("");
...
*/
	...
	$("#VentaRepreDestinoRecibo").chosen({
		no_results_text           : 'No se encuentra el destino.',
		width                     : "95%",
		allow_single_deselect     : true, 
		search_contains           : true,
	...
	$("#VentaRepreDestino").change(function(){
		if($("#VentaRepreOrigen").val() == ""){
			...
			if($(this).val() != ""){
				...
				$.ajax({
					type: 'json',
					url: fullpath+"VentaRepre/getTarifa/"+clienteId+"/"+origenId+"/"+destinoId,
