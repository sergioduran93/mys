...
<div class="row" style="width:90%; margin-left:5%;">
	...
	<?php echo $this->Form->button("Guardar",array('id'=>'btn-submit',"class"=>'btn btn-primary',"style"=>'float:right;margin-bottom: 15px;'));?>
...
<script>
	$("#btn-submit").click(function(){
		if(submit){
			itsclicked = true;
			$("#VentasCreditoRepreCrearForm").submit();
		} else {
	...
	function calcularTotal(){
		...
		if(ajaxCall != undefined){
			var auxi = ajaxCall.Cupo - (ajaxCall.Saldo + op);
			if(auxi < 0){
				alert("El cliente no tiene suficiente cupo, faltante: $"+(-1*auxi));
				submit = false;
			} else {
				submit = true;
			}
