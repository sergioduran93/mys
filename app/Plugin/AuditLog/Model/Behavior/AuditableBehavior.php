...
class AuditableBehavior extends ModelBehavior {
  ...
  public function setup( Model $Model, $settings = array() ) {
    if( !isset( $this->settings[$Model->alias] ) ) {
      $this->settings[$Model->alias] = array(
        'ignore' => array('id', 'created', 'updated', 'modified' ),
  ...
  public function afterSave( Model $Model, $created , $options = array() ) {
    $audit = array( $this->_getModelData( $Model ) );
    ...
    $source = $Session->read('Auth.User');
    $source['id'] = $source['name'].' '.$source['lastname'];
    $data = array(
      'Audit' => array(
        'event'     => $created ? 'CREADO' : 'EDITADO',
        'model'     => $Model->alias,
        'entity_id' => $Model->id,
        'json_object' => json_encode( $audit ,false),
  ...
  public function afterDelete( Model $Model ) {
    ...
    $audit = array( $this->_original[$Model->alias] );
    $data  = array(
      'Audit' => array(
        'event'       => 'ELIMINADO',
        'model'       => $Model->alias,
