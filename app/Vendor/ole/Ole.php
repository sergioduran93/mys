...
class Ole
{
    ...
    */
    public function read($file)
    {
        ...
        $fh = @fopen($file, "r");
        if ($fh == false) {
            trigger_error("Can't open file $file", E_ERROR);
        }
        ...
        if ("\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1" != $signature) {
            trigger_error("File doesn't seem to be an OLE container.", E_ERROR);
        }
    ...
    * @return mixed true on success, PEAR_Error on failure
    */
    private function _readPpsWks($pps_wk_start, $big_block_size)
    {
        ...
        while (1)
        {
            ...
            if (strlen($pps_wk) != OLE_PPS_SIZE) {
                ...
                //return trigger_error("PPS at $pointer seems too short: ".strlen($pps_wk));
            }
            ...
            {
                trigger_error("PPS at $pointer has unknown type: {$type['']}", E_ERROR);
            }
            ...
            $this->_list[] = new OlePps(null, '', $type[''], $prev[''], $next[''],
    ...
    */
    private function _ppsTreeComplete($index)
    {
    ...
    */
    ...
    /**
    * Utility public function to transform ASCII text to Unicode
    *
    * @access public
    * @static
    ...
    */
    ...
    {
        if (strlen($string) != 8) {
            trigger_error("Expecting 8 byte string", E_USER_ERROR);
        }
